...
@Service
final class SecurityServiceImpl_old implements SecurityService {
	private static final Logger logger = LoggerFactory.getLogger(com.isure.security.SecurityServiceImpl_old.class);
	...
	public ISureSecurityContext getRequestSecurityContext(HttpServletRequest request) {
		...
		if (request.getAttribute(com.isure.security.SecurityServiceImpl_old.SECURITY_CONTEXT_KEY) != null) {
			//  security context already set - use it
			result = (ISureSecurityContext) request.getAttribute(com.isure.security.SecurityServiceImpl_old.SECURITY_CONTEXT_KEY);
		} else {
			...
			if (cookie != null && !cookie.isEmpty()) {
				com.isure.security.SecurityServiceImpl_old.ISurePrincipal token = extractSecurityToken(cookie);
				if (token != null) {
